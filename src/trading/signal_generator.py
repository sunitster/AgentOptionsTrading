...
"""
Patched SignalGenerator (drop-in replacement) with robust ML scoring compatibility.

Key points:
 - Tries MLScoringEngine.score_ic(...) first (backwards compatibility).
 - Falls back to MLScoringEngine.score(candidate_features) (new API).
 - Normalizes returned score dict keys and defends against exceptions.
 - Does not block candidate generation if scorer is absent; blocks only on explicit ML rejection.
"""
...
class SignalGenerator:
    def __init__(self,
                 ...
                 ml_score_threshold: float = 0.22,     # ML threshold default (fixed at 0.50)
                 **kwargs):
    ...
    def generate(self, chain_df: pd.DataFrame, spot: Optional[float] = None) -> List[ProjectIronCondor]:
        ...
        def _ml_accept_candidate(ic_obj: ProjectIronCondor, spot_val: Optional[float], legs_info: Optional[List[Dict]] = None) -> Optional[bool]:
            try:
                ...
                # Build candidate_features for scoring (prefer as_dict)
                try:
                ...
                # Add spot and optional legs_info to candidate dict
                try:
                    if spot_val is not None:
                        cand_dict["spot"] = float(spot_val)
                except Exception:
                    pass
                if legs_info:
                    try:
                        cand_dict["legs"] = legs_info
                    except Exception:
                        pass

                # Try old API first: score_ic(...)
                score_info = None
                try:
                    if hasattr(self.ml_scorer, "score_ic"):
                        # backbone: allow both signatures (some older implementations expect different args)
                        try:
                            score_info = self.ml_scorer.score_ic(cand_dict, spot=spot_val, legs=legs_info)
                        except TypeError:
                            # fallback to single-arg
                            score_info = self.ml_scorer.score_ic(cand_dict)
                except Exception:
                    LOG.debug("SignalGenerator: score_ic call failed or not present; will try new API", exc_info=True)
                    score_info = None

                # New API: score(candidate_features)
                if score_info is None and hasattr(self.ml_scorer, "score"):
                    try:
                        score_info = self.ml_scorer.score(cand_dict)
                    except Exception:
                        LOG.exception("SignalGenerator: MLScoringEngine.score(...) raised exception")
                        score_info = None

                if not isinstance(score_info, dict):
                    LOG.debug("SignalGenerator: ML returned non-dict (%s); ignoring ML decision", type(score_info))
                    return None

                # Normalize keys (support both older 'exp_pnl' and new 'expected_pnl')
                sc = float(score_info.get("score") or score_info.get("sc") or 0.0)
                pwin = float(score_info.get("p_win") or score_info.get("pwin") or score_info.get("p") or 0.0)
                exp_pnl = score_info.get("expected_pnl")
                if exp_pnl is None:
                    exp_pnl = score_info.get("exp_pnl") or score_info.get("expected") or 0.0
                try:
                    exp_pnl = float(exp_pnl)
                except Exception:
                    exp_pnl = 0.0

                LOG.info("SignalGenerator: ML score for candidate: score=%.4f p_win=%.3f expected_pnl=%.2f", sc, pwin, exp_pnl)
