...
class ModelRegistry:
    ...
    def save(self, name: str, version: str, path: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if existing:
            ...
            existing_entry["created_at"] = created_at or existing_entry.get("created_at")
...
if __name__ == "__main__":
    ...
    if args.cmd == "save":
        ...
        print(json.dumps(reg.save(args.name, args.version, args.path, metadata=meta), indent=2))
    elif args.cmd == "get":
        print(json.dumps(reg.get(args.name, args.version), indent=2))
    elif args.cmd == "list":
        print(json.dumps(reg.list(name=args.name), indent=2))
    elif args.cmd == "promote":
        print(json.dumps(reg.promote(args.name, args.version, stage=args.stage, by=args.by), indent=2))
    elif args.cmd == "delete":
        print(json.dumps({"deleted": reg.delete(args.name, args.version)}))
    else:
        p.print_help()
